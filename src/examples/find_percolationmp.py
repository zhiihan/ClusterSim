...
def main(input_vec):
    ...
    for p_index, p in enumerate(p_vec):
        ...
        for seed in range(samples):
            with open(
                f"./data/cc{p:.4f}shape{shape[0]}{shape[1]}{shape[2]}sample{seed}round{rounds}",
                "rb",
            ) as f:
                try:
                    cc = pickle.load(f)
                    ...
                    if percol_dist >= shape[0] - 3:
                        print(
                            f"dist={percol_dist}, p={p}, seed={seed}, percolates, round={rounds}"
                        )
                    else:
    ...
    for i in results:
        ...
        if rounds < 5:
            plt.plot(
                plot_data[:, 0],
                plot_data[:, 1],
                label=f"r = {rounds+1} rounds, shape = {shape}",
            )
        else:
            plt.plot(
                plot_data[:, 0],
                plot_data[:, 1],
                label=f"r = {rounds} rounds, shape = {shape}",
            )
        plt.legend()
